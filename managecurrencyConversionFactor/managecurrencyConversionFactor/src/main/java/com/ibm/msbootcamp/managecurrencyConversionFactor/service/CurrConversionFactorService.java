...
public double convertCurrencyToRupee(String countryCode, double amount)
{
	...
	/*
	 * CurrencyFactor currencyFactor = new CurrencyFactor(); RestTemplate template =
	 * new RestTemplate(); HttpEntity<CurrencyFactor> reqEntity = new
	 * HttpEntity<CurrencyFactor>(currencyFactor);
	 * HttpEntity<ConversionFactorResponse> conversionFactorResponse =
	 * template.exchange("", HttpMethod.POST, reqEntity,
	 * ConversionFactorResponse.class);
	 */
	//ConversionFactorResponse convFactorResponse = conversionFactorResponse.getBody();
	double convertedAmount = convFactor*amount;
